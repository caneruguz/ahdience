...
app.controller = function(){
    ...
    this.prev = function(){
        ...
        if(self.currentSlide > 0 ){
            //if there are still data-index elements on this page
            var slide = $('.slide[data-id='+self.currentSlide+']')
            if(self.currentItem > -1) {
                slide.find('*[data-index='+self.currentItem+']').hide();
                self.currentItem--;
            } else {
                self.currentSlide--;
                var slide = $('.slide[data-id='+self.currentSlide+']')
                var totalItems = slide.find('*[data-index]:last').attr('data-index');
                self.currentItem = totalItems;
    ...
    this.next = function() {
        ...
        if(self.currentSlide < self.settings.totalSlides ){
            //if there are still data-index elements on this page
            var slide = $('.slide[data-id='+self.currentSlide+']')//.find('[data-index]:last-of-type').attr('data-index')
            var totalItems = slide.find('*[data-index]:last').attr('data-index');
            console.log("totalItems", parseInt(totalItems));
            if(self.currentItem < parseInt(totalItems)+1) {
                slide.find('*[data-index='+self.currentItem+']').show();
                self.currentItem++;
            } else {
                self.currentItem = 0;
                self.currentSlide++;
